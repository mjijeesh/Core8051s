...
proc createBfmInstr {command size waitCount} {
    ...
    # Write CoreMP7 BFM instruction to instr.vec file
...
if [ catch { open $bfmCompileLogFile w } bfmCompile_log ] {
    puts "Error: CoreMP7 BFM Compile: Cannot write BFM compiler log file \"$bfmCompileLogFile\""
}

if [ catch { open $addrVectors w } transAddr ] {
    puts "Error: CoreMP7 BFM Compile: Cannot write transAddr.vec file \"$addrVectors\""
    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot write transAddr.vec file \"$addrVectors\""
}

if [ catch { open $dataVectors w } transData ] {
    puts "Error: CoreMP7 BFM Compile: Cannot write transData.vec file \"$dataVectors\""
    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot write transData.vec file \"$dataVectors\""
}

if [ catch { open $instrVectors w } transInstr ] {
    puts "Error: CoreMP7 BFM Compile: Cannot write transInstr.vec file \"$instrVectors\""
    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot write transInstr.vec file \"$instrVectors\""
}

if [ catch { open $numTransVectors w } numTrans ] {
    puts "Error: CoreMP7 BFM Compile: Cannot write numTrans.vec file \"$numTransVectors\""
    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot write numTrans.vec file \"$numTransVectors\""
}

if [ catch { open $numErrorsFile w } numErrors_file ] {
    puts "Error: CoreMP7 BFM Compile: Cannot write numErrors.vec file \"$numErrorsFile\""
    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot write numErrors.vec file \"$numErrorsFile\""
}

puts "CoreMP7 BFM Compiler v$compilerVersion"
puts $bfmCompile_log "CoreMP7 BFM Compiler v$compilerVersion"
...
# Open the raw CoreMP7 BFM script
if [ catch { open "$bfmScript" r } script ] {
	puts "Error: CoreMP7 BFM Compile (Pre-process): Cannot read file \"$bfmScript\""
	puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot read file \"$bfmScript\""
	set errorVar 1
	return 0
}
...
if [ catch { open $bfmScript_i w } script_i ] {
    puts "Error: CoreMP7 BFM Compile (Pre-process): Cannot write intermediate BFM script file \"$bfmScript_i\""
    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot write intermediate BFM script file \"$bfmScript_i\""
}
...
while { [ gets $script line ] >= 0 } {
	...
	switch -regexp -- $line {
            ...
            switch -exact $command {
                "memmap" {
                    ...
                    if { [expr ($validCommand == 1)] } {
	                    ...
	                    puts "Error: CoreMP7 BFM Compile (Pre-process): \"$bfmScript\" Line $lineNumber: Invalid command "
	                    puts $bfmCompile_log "Error: CoreMP7 BFM Compile (Pre-process): \"$bfmScript\" Line $lineNumber: Invalid command "
                    ...
                    if { [expr ($validCommand == 1)] } {
                        if { [expr ([ lsearch -exact [ array names base ] [ string tolower $resourceInstance ] ]) != (-1)] } {
                            ...
                            if [ catch { open "$resourceScriptlet" r } scriptlet ] {
	                            puts "Error: CoreMP7 BFM Compile (Pre-process): Cannot read scriptlet file \"$resourceScriptlet\""
	                            puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot read scriptlet file \"$resourceScriptlet\""
	                            set errorVar 1
	                            return 0
                            }
	                        ...
	                        puts "Error: CoreMP7 BFM Compile (Pre-process): \"$bfmScript\" Line $lineNumber: Unrecognized resource instance: \"$resourceInstance\""
	                        puts $bfmCompile_log "Error: CoreMP7 BFM Compile (Pre-process): \"$bfmScript\" Line $lineNumber: Unrecognized resource instance: \"$resourceInstance\""
                            set numErrors [ expr ($numErrors + 1) ]
                        }
                    } else {
	                    puts "Error: CoreMP7 BFM Compile (Pre-process): \"$bfmScript\" Line $lineNumber: Invalid command "
		...
		default {
	        puts "Error: CoreMP7 BFM Compile (Pre-process): \"$bfmScript\" Line $lineNumber: Invalid BFM script command"
	        puts $bfmCompile_log "Error: CoreMP7 BFM Compile: \"$bfmScript\" Line $lineNumber: Invalid BFM script command"
            set numErrors [ expr ($numErrors + 1) ]
		}
	}
}
...
if { [expr ($numErrors > 0)]} {
	puts "CoreMP7 BFM Compile (Pre-process): $numErrors error(s) encountered"
	puts "Please fix BFM script and re-run simulation"
	puts " - see bfmCompile.log for details of BFM script errors"
	puts $bfmCompile_log "CoreMP7 BFM Compile (Pre-process): $numErrors error(s) encountered"
    ...
    # Open the pre-processed CoreMP7 BFM script
    if [ catch { open "$bfmScript_i" r } script_i ] {
	    puts "Error: CoreMP7 BFM Compile: Cannot read file \"$bfmScript_i\""
	    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: Cannot read file \"$bfmScript_i\""
	    set errorVar 1
	    return 0
    }
	    ...
	    switch -regexp -- $line {
		    ...
		    {^([ \t]*)([a-zA-Z]+)([ \t]*)} {
                ...
                switch -exact $command {
                    ...
                    "read" {
                        ...
                        if { [expr ($validCommand == 1)] } {
                            if { [expr ([ lsearch -exact [ array names base ] $resourceInstance ]) != (-1)] } {
	                            ...
	                            puts "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Unrecognized resource instance: \"$resourceInstance\""
	                            puts $bfmCompile_log "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Unrecognized resource instance: \"$resourceInstance\""
                                set numErrors [ expr ($numErrors + 1) ]
                            }
                        } else {
	                        puts "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Invalid command on bus cycle $vecCount "
	                        puts $bfmCompile_log "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Invalid command on bus cycle $vecCount "
	                        puts "\t\t - allowed widths are b (byte), h (halfword), w (word) "
	                        puts "\t\t - all numbers must be in hex (preceded by 0x) "
                            set numErrors [ expr ($numErrors + 1) ]
                        }
                    ...
                    default {
	                    puts "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Invalid BFM script command: \"$command\""
	                    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Invalid BFM script command: \"$command\""
                        set numErrors [ expr ($numErrors + 1) ]
                    }
                }
            }
		    default {
	            puts "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Invalid BFM script command"
	            puts $bfmCompile_log "Error: CoreMP7 BFM Compile: \"$bfmScript_i\" Line $lineNumber: Invalid BFM script command"
                set numErrors [ expr ($numErrors + 1) ]
		    }
	    }
    }
    ...
    if { [expr ($vecCount > 1024)]} {
	    puts "Error: CoreMP7 BFM Compile: BFM script contains more than maximum 1024 bus transactions"
	    puts $bfmCompile_log "Error: CoreMP7 BFM Compile: BFM script contains more than maximum 1024 bus transactions"
        set numErrors [ expr ($numErrors + 1) ]
    } else {
    ...
    if { [expr ($numErrors > 0)]} {
	    puts "CoreMP7 BFM Compile: $numErrors error(s) encountered"
	    puts "Please fix BFM script and re-run simulation"
	    puts " - see bfmCompile.log for details of BFM script errors"
	    puts $bfmCompile_log "CoreMP7 BFM Compile: $numErrors error(s) encountered"
	    puts $bfmCompile_log "Please fix BFM script and re-run simulation"
    } else {
