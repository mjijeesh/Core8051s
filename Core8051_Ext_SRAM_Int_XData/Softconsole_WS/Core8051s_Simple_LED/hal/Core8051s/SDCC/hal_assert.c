...
)
{
    ...
    for ( msg_length = 0; (msg_length < MAX_PATH_FILE_NAME) && (file_name[msg_length] != 0); msg_length++ )
    {
        ;
    }

    UART_send( &g_ext_uart,	file_name, msg_length );
...
)
{
    while(1)
    {
        ...
        volatile uint8_t dummy;        dummy = *assert_file;
    }
}
